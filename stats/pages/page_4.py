...
# dropping cols
df_luiss_merged = df_luiss_merged.drop(columns=['WL_opponent', 'HA_opponent', 'SEASON_LEVEL_opponent', 'PIR', 'EFF', '+/-', 'No', 'Player', 'MIN', 'Best Partial'])
...
# --- HA Table 1 ---
df_match_luiss_melt_ha = df_match_luiss.melt(id_vars='HA', var_name='Statistica', value_name='Valore')
...
df_match_luiss_grouped_ha = (
    df_match_luiss_melt_ha
    .loc[~df_match_luiss_melt_ha["Statistica"].isin(exclude_stats_ha)]
    .groupby(["HA", "Statistica"], as_index=False)["Valore"]
    .mean()
    .round(2)
)
df_match_luiss_pivot_ha = df_match_luiss_grouped_ha.pivot(index='Statistica', columns='HA', values='Valore').reset_index()
print("Pivot HA:", df_match_luiss_pivot_ha.shape)

# --- WL Table 1 ---
df_match_luiss_melt_wl = df_match_luiss.melt(id_vars='WL', var_name='Statistica', value_name='Valore')
# grouping the melted df by HA and Statistica and summing Valore where the statistica is not Best Partial,OPPONENT,WL,SEASON_LEVEL,DATA
df_match_luiss_grouped_wl = (
    df_match_luiss_melt_wl
    .loc[~df_match_luiss_melt_wl["Statistica"].isin(exclude_stats_wl)]
    .groupby(["WL", "Statistica"], as_index=False)["Valore"]
    .mean()
    .round(2)
)
df_match_luiss_pivot_wl = df_match_luiss_grouped_wl.pivot(index='Statistica', columns='WL', values='Valore').reset_index()
print("Pivot WL:", df_match_luiss_pivot_wl.shape)

# --- HA Table 2 ---
df_luiss_melt_ha = df_luiss.melt(id_vars='HA', var_name='Statistica', value_name='Valore')

df_luiss_grouped_ha = (
    df_luiss_melt_ha
    .loc[~df_luiss_melt_ha["Statistica"].isin(exclude_stats_ha_2)]
    .groupby(["HA", "Statistica"], as_index=False)["Valore"]
    .mean()
    .round(2)
)
...
df_wide_ha = df_luiss_grouped_ha.pivot(index="HA", columns="Statistica", values="Valore")
...
# --- Combiniamo ---
df_luiss_grouped_ha = pd.concat([df_luiss_grouped_ha, df_percentuali_ha], ignore_index=True)

# --- Pivot finale (se ti serve per la visualizzazione)
df_luiss_pivot_ha = df_luiss_grouped_ha.pivot(index='Statistica', columns='HA', values='Valore').reset_index()

print("Pivot HA:", df_luiss_pivot_ha.shape)


# --- WL Table 2 ---
df_luiss_melt_wl = df_luiss.melt(id_vars='WL', var_name='Statistica', value_name='Valore')
df_luiss_grouped_wl = (
    df_luiss_melt_wl
    .loc[~df_luiss_melt_wl["Statistica"].isin(exclude_stats_wl_2)]
    .groupby(["WL", "Statistica"], as_index=False)["Valore"]
    .mean()
    .round(2)
)
...
# --- Calcolo percentuali 2P% e 3P% ---
# Trasformiamo in wide format per accedere facilmente ai valori
df_wide_wl = df_luiss_grouped_wl.pivot(index="WL", columns="Statistica", values="Valore")
# Calcolo percentuali (evita divisione per 0)
...
# --- Combiniamo ---
df_luiss_grouped_wl = pd.concat([df_luiss_grouped_wl, df_percentuali_wl], ignore_index=True)

# --- Pivot finale (se ti serve per la visualizzazione)
df_luiss_pivot_wl = df_luiss_grouped_wl.pivot(index='Statistica', columns='WL', values='Valore').reset_index()

print("Pivot HA:", df_luiss_pivot_wl.shape)
...
def update_graph(home_away, win_lose, statistica):
    ...
    # --- Filter for graph ---
    df_filtered = df_luiss_merged[
        df_luiss_merged['HA_match'].isin(home_away) &
        df_luiss_merged['WL_match'].isin(win_lose)
    ]
    ...
    data_ha = df_match_luiss_pivot_ha.to_dict('records')
    columns_ha = [{"name": i, "id": i, "type": "numeric", "format": Format(precision=2, scheme=Scheme.fixed)} 
                  for i in df_match_luiss_pivot_ha.columns]
    data_wl = df_match_luiss_pivot_wl.to_dict('records')
    columns_wl = [{"name": i, "id": i, "type": "numeric", "format": Format(precision=2, scheme=Scheme.fixed)} 
                  for i in df_match_luiss_pivot_wl.columns]

    data_ha_2 = df_luiss_pivot_ha.to_dict('records')
    columns_ha_2 = [{"name": i, "id": i, "type": "numeric", "format": Format(precision=2, scheme=Scheme.fixed)} 
                    for i in df_luiss_pivot_ha.columns]
    data_wl_2 = df_luiss_pivot_wl.to_dict('records')
    columns_wl_2 = [{"name": i, "id": i, "type": "numeric", "format": Format(precision=2, scheme=Scheme.fixed)} 
                    for i in df_luiss_pivot_wl.columns]
